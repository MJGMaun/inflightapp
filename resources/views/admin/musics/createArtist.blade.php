...
<script type="text/javascript">
        ...
        $('a.add_album').click(function(e) {
            ...
            if ($('#album input').length < 5) {
                $('#album').append('<div class="input">{{Form::text('albums[]', '', ['class' => 'form-control album', 'placeholder' => 'Album Name'])}}<br><div class="row"><div class="col-md-6 col-sm-6">{{Form::label('categories[]', 'Category')}} {{Form::select('categories[]', ['1' => 'Top Albums of The Month','2' => 'New Album Released', '3' => 'Popular Album', '0' => 'None'], null, ['class' => 'form-control', 'placeholder' => 'Select a category...'])}}</div><div class="col-md-6 col-sm-6"><div class="image-container"><div class="image">{{Form::label('cover_image[]', 'Cover Image')}}<br>{{Form::file('cover_image[]')}}</div></div></div></div><br><div class="row"><div class="col-md-6 col-sm-6">{{Form::label('album_description[]', 'Album Description')}} {{Form::textarea('album_description[]', '', ['id' => 'album_description', 'class' => 'form-control','placeholder' => 'Enter album description here', 'rows' => '2'])}}</div><div class="col-md-6 col-sm-6"><div class="image-container"><div class="image">{{Form::label('release_date[]', 'Release Date')}} {{Form::date('release_date[]', null, ['class' => 'form-control'])}}</div></div></div></div><hr><br></div>');
            }
        });
        ...
        $('select[name="artists"]').change(function(){
            ...
            if ($(this).val() == "new_artist"){
                ...
                $('textarea[name="album_description[]"]').attr("disabled","disabled");
                $('input[name="release_date[]"]').attr("disabled","disabled");
                $('input[name="albums[]"]').attr("disabled","disabled");
        ...
        //  var val = $(this).parent().find("input").val();         
         ...
         $('textarea[name="album_description[]"]').removeAttr('disabled');
         $('input[name="release_date[]"]').removeAttr('disabled');
